...
def evaluate(type, global_step=None):
    ...
    with graph.as_default() as g:
        ...
        if type == 'train':
            filenames = [os.path.join(scvs_input.DATA_DIR, 'image_' + str(6*i+j) + '.tfrecords')
        ...
        patches_y, labels = scvs.inputs(filenames=filenames)

        # Build a Graph that computes the logits predictions from the
        # inference model.
        scores = scvs.inference(patches_y, keep_prob)

        total_loss = scvs.loss_func(scores, labels)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        if type == 'val':
            checkpoint_file = os.path.join(scvs_input.LOG_DIR5, 'temp_model.ckpt')
        else:
            checkpoint_file = os.path.join(scvs_input.LOG_DIR5, 'temp_model.ckpt')
...
def train():
    ...
    with graph.as_default():
        ...
        # Input images and labels.
        filenames = [os.path.join(scvs_input.DATA_DIR5, 'image_' + str(6*i+j) + '.tfrecords')
                     for i in ORDER[0:scvs_input.TRAIN_DATA_NUM] for j in range(6)]
        patches_y, moses, labels  = scvs.distorted_inputs_jjy_5(filenames)

        print(patches_y, moses, labels)

        # Build a Graph that computes predictions from the inference model.
        scores = scvs.inference_jjy_5( patches_y,labels, keep_prob)
    ...
    with tf.Session(graph=graph) as sess:
        ...
        summary_writer = tf.summary.FileWriter(scvs_input.LOG_DIR5, sess.graph)
        ...
        for step in range(scvs.MAX_STEPS):
            _, loss_value, gt_labels = sess.run([train_op, total_loss, labels], feed_dict={keep_prob: 0.5})
            ...
            if step % (iters_per_epoch*10) == 0 or (step + 1) == scvs.MAX_STEPS:
                ...
                if val_loss < min_loss:
                    ...
                    checkpoint_file = os.path.join(scvs_input.LOG_DIR5, 'best_model.ckpt')
                    saver1.save(sess, checkpoint_file)
