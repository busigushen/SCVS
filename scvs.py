...
def distorted_inputs_jjy_5(filenames):
    ...
    """
    patches_y, moses, patches_labels = scvs_input.distorted_inputs_jjy_5(filenames=filenames, batch_size=BATCH_SIZE)

    return patches_y, moses,patches_labels


def distorted_inputs_jjy_6(filenames):
    """Construct distorted input for training using the Reader ops.
    ...
    Raises:
        :exception ValueError: If no data_dir
    """
    patches_y, moses,patches_vs  = scvs_input.distorted_inputs_jjy_6(filenames=filenames, batch_size=BATCH_SIZE)
...
def inputs(filenames):
    ...
    Raises:
        :exception ValueError: If no data_dir
    """
    patches_y, labels = scvs_input.inputs(filenames=filenames, batch_size=BATCH_SIZE)
...
def inference(patches_y, keep_prob):
    ...
    with tf.variable_scope('fuse_features'):
        ...
        features = tf.reshape(tensor=feature_map_y, shape=[scvs_input.NUM_PATCHES_PER_IMAGE * BATCH_SIZE, -1])
    ...
    with tf.variable_scope('estimate_quality') as scope:
        patches_qualities = tf.reshape(patches_qualities, (BATCH_SIZE, scvs_input.NUM_PATCHES_PER_IMAGE))

        output = tf.reduce_mean(patches_qualities, axis=1)

    return output


def inference_jjy_4(patches_y, moses,patches_vs, keep_prob):
    """Build the model up to where it may be used for inference.
    ...
    with tf.variable_scope('fuse_features'):
        ...
        features = tf.reshape(tensor=feature_map_y, shape=[-1, feature_map_y.get_shape()[1].value*feature_map_y.get_shape()[2].value*feature_map_y.get_shape()[3].value])
    ...
    with tf.variable_scope('estimate_quality') as scope:
        print(patches_qualities)
        patches_qualities_reshape=tf.reshape(patches_qualities,[-1,3,3])
        kernel= numpy.array([[9/16,9/8,9/16],[9/8,9/4,9/8],[9/16,9/8,9/16]])
        kernel=tf.constant(kernel, tf.float32)
        
        patches_qualities_mul=tf.multiply(patches_qualities_reshape,kernel)
        patches_qualities_pool=tf.reduce_mean(patches_qualities_mul,axis=(1,2))
        
        patches_qualities_reshape_2=tf.reshape(patches_qualities_pool,[BATCH_SIZE,-1])  ## 这里的第一维是batchsize，第二维是每一张图取的patch

        patches_vs=tf.reshape(patches_vs ,[BATCH_SIZE,-1])
        patches_vs_sum=tf.reduce_sum(patches_vs,axis=1,keep_dims=True)

        patches_vs_sum=tf.tile(patches_vs_sum,[1,18])
        
        patches_vs_nor=tf.divide(patches_vs,patches_vs_sum)

        output=tf.multiply(patches_qualities_reshape_2,patches_vs_nor)
        output=tf.reduce_sum(output,axis=1)              

    return output


def inference_jjy_5 (patches_y,labels, keep_prob):
    """Build the model up to where it may be used for inference.
